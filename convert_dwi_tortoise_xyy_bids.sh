...
g01_num=`cat ${dicom_readme} | grep "edti_2mm_cdif09_g01" | cut -d "," -f 3 | cut -d ":" -f 2`
g02_num=`cat ${dicom_readme} | grep "edti_2mm_cdif09_g02" | cut -d "," -f 3 | cut -d ":" -f 2`
g03_num=`cat ${dicom_readme} | grep "edti_2mm_cdif09_g03" | cut -d "," -f 3 | cut -d ":" -f 2`
g04_num=`cat ${dicom_readme} | grep "edti_2mm_cdif09_g04" | cut -d "," -f 3 | cut -d ":" -f 2`
...
g04_dicom_dir=${dicom_dir}/sub-${sub}/ses-${ses}/*/${g04_num}
gradients_file=${dicom_dir}/sub-${sub}/ses-${ses}/*/realtime/cdiflist09
...
then
    ...
    mkdir $dwi_dicom_sub/dicom/g04_dicom_dir
    ...
    ln -s $g04_dicom_dir/* $dwi_dicom_sub/dicom/g04_dicom_dir
    ln -s $gradients_file $dwi_dicom_sub/cdiflist09

    # run tortoise --> get the mri_ap_proc and mri_pc_proc
    ImportDICOM -i $dwi_dicom_sub/dicom -g $dwi_dicom_sub/cdiflist09 -b 1100
